...
function hookCreateScript(matcher, transformer, opts) {
    ...
    var orgScript = vm.Script,
        orgRunInContext = orgScript.prototype.runInContext,
        orgRunInNewContext = orgScript.prototype.runInNewContext;

    orgScript.prototype.originalRunInContext = orgRunInContext;
    orgScript.prototype.originalRunInNewContext = orgRunInNewContext;

    orgScript.prototype.runInContext = function(context, options) {
        Object.keys(global).forEach(function (key) {
            context[key] = global[key];
        });
        return this.originalRunInContext(context, options);
    };
    orgScript.prototype.runInNewContext = function(context, options) {
        Object.keys(global).forEach(function (key) {
            context[key] = global[key];
        });
        return this.originalRunInNewContext(context, options);
    }
